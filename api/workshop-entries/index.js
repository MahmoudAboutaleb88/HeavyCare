// api/workshop-entries/index.js
//
// GET  /api/workshop-entries          -> list entries (default: open ones)
// POST /api/workshop-entries          -> register equipment entering the workshop
//
// Business rules enforced here:
//   1. A piece of equipment can only have ONE open workshop entry at a time
//      (you can't "enter" the same equipment twice before it exits).
//   2. Creating an entry automatically:
//        - creates its matching job_cards row (the "maintenance file")
//        - sets equipment.current_status = 'in_workshop'
//   3. duration_hours / duration_days are NOT set here — they only get
//      calculated when the entry is closed (next module).

const { query, withTransaction } = require('../_lib/db');
const { requireAuth } = require('../_lib/auth');
const { sendSuccess, sendError } = require('../_lib/http');

// Everyone except 'viewer' can open a workshop entry — this is daily
// operational work done by technicians and supervisors, not just managers.
...
async function handleCreate(req, res, user) {
  try {
    ...
    if (!['PM', 'BD'].includes(maintenanceType)) {
      return sendError(res, 400, 'Maintenance type must be PM or BD');
    }
    ...
    const equipmentRows = await query(
      'SELECT id, is_archived FROM equipment WHERE id = ?',
      [equipmentId]
    );
    ...
    // Rule: no second open entry for equipment that's already in the workshop.
    ...
    // Everything below must succeed together, or none of it should —
    // hence the transaction: entry + job card + equipment status update.
    const entryId = await withTransaction(async (conn) => {
      ...
      // Human-friendly entry number, generated from the real id so it's
      // guaranteed unique: WE-2026-0000042
      const year = entryDate.slice(0, 4);
      const entryNumber = 'WE-' + year + '-' + String(newId).padStart(6, '0');

      await conn.execute(
        'UPDATE workshop_entries SET entry_number = ? WHERE id = ?',
        [entryNumber, newId]
      );

      // Auto-create the matching "maintenance file" (job card) for this entry.
      await conn.execute(
        `INSERT INTO job_cards (workshop_entry_id, progress_status) VALUES (?, 'waiting')`,
        [newId]
      );

      // The equipment is now physically inside the workshop.
      await conn.execute(
        `UPDATE equipment SET current_status = 'in_workshop' WHERE id = ?`,
        [equipmentId]
      );
